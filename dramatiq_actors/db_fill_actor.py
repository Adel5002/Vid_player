...
async def fill_db(season: str = f'{date.today().year}'):
    ...
    cache.set("TOTAL_TASKS", 0)
    cache.set("DONE_TASKS", 0)
    ...
    for page in range(1, 10):
        ...
        for anime in safe_response.get('animes'):
            cache.incr("TOTAL_TASKS")
            add_anime_to_db.send(anime)

    print("✅ Все задачи отправлены, ждем завершения...")
...
async def add_anime_to_db(anime_data: dict) -> None:
    ...
    try:
        ...
        done = int(cache.get("DONE_TASKS") or 0)

        if done >= total and total > 0:
            cache.set("DB_READY", "true")
            print("🎉 Все задачи завершены! DB_READY = true")
